"""
================================================================================
PCON MANIFEST SYSTEM - CONSOLIDATED VERSION 2.0 (UPDATED)
================================================================================
Updated with Stop Selector Dropdown Feature

CHANGES IN THIS VERSION:
- Added stop selector dropdown to choose which stop to work on
- "Working on Stop" header shows current stop details
- Can switch between stops easily to add/edit shipments
- Button changed from "Add Stop" to "Add New Stop"
...
# ===============================================================================
# SECTION 6: UI COMPONENTS MODULE - UPDATED WITH STOP SELECTOR
# ================================================================================
...
class UIComponents:
    ...
    def render_create_mode(self):
        ...
        try:
            ...
            if manifest_info:
                ...
                # Stop management (includes selector and form)
    ...
    # ==================== NEW: Stop Selector Dropdown ====================

    def _render_stop_selector(self):
        """Render stop selector dropdown to choose which stop to work on"""
        try:
            manifest_no = self.session.get_current_manifest()
            if not manifest_no:
                return

            # Get all stops for current manifest
            stops = self.db.get_stops_for_manifest(manifest_no)

            if stops.empty:
                st.info("No stops added yet. Add your first stop above.")
                return
            ...
            for _, stop_row in stops.iterrows():
                ...
                label = f"Stop {int(stop_row['STOP_ORDER'])} - {stop_row['CODE_DESTINATION'] or 'N/A'} - {stop_row['SHIPVIA'] or 'N/A'}"
                stop_options[label] = drop_no

            # Get current stop to set default
            current_stop = self.session.get_current_stop()
            
            # Find the index of current stop in options
            default_index = 0
            if current_stop and current_stop in stop_options.values():
                labels_list = list(stop_options.keys())
                values_list = list(stop_options.values())
                try:
                    default_index = values_list.index(current_stop)
                except ValueError:
                    default_index = 0

            # Dropdown to select stop
            selected_label = st.selectbox(
                "Choose a stop to add shipments:",
            ...
            # Update current stop if changed
            if selected_drop_no != current_stop:
                self.session.set_current_stop(selected_drop_no)
                self.logger.log_info(f"Switched to working on stop {selected_drop_no}")
                st.rerun()

            # Display "Working on Stop" header with details
            if selected_drop_no:
                selected_stop = stops[stops['DROP_NO'] == selected_drop_no].iloc[0]
                
                st.markdown("---")
                st.markdown("### 🎯 Working on Stop")
                col1, col2, col3 = st.columns(3)
                
                with col1:
                    st.write(f"**Stop Order:** {int(selected_stop['STOP_ORDER'])}")
                
                with col2:
                    st.write(f"**Destination:** {selected_stop['CODE_DESTINATION'] or 'N/A'}")
                
                with col3:
                    st.write(f"**Ship Via:** {selected_stop['SHIPVIA'] or 'N/A'}")
                
                st.markdown("---")

        except Exception as e:
            self.logger.log_error("Error rendering stop selector", e)
            st.error("Error loading stop selector")

    def _render_stop_form(self):
        """Render stop creation form with selector"""
        ...
        if st.button("Add New Stop"):
            try:
                ...
                st.success(f"Stop added successfully!")
        ...
        # Render stop selector if stops exist
